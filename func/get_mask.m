function [mask]=get_mask(I, threshold)
...
Im_max = prctile(abs(Img0(:)), 99);  %wmq
    Im_mean = mean(mean(abs(Img0(:,:))));
% prctile(Img0(:), 99.8)
%     [FX,FY] = GRADIENT(Img0);
    [FX,FY] = gradient(Img0);
    FZ=abs(FX)+abs(FY);
...
%     Img_Hist=Img0>=Im_max*8/90;     %20
    
%     Img_Hist=Img0>=Im_max*4/90;     %by victor for leg
    Img_Hist=Img0>=Im_mean*threshold;   %2/3 for leg(larger one)
% Img_Hist=Img0>=Im_max*threshold;
...
%     maska = bwselect(maska,round(Km/2), round(Kn/2),8);
    maska1 = bwselect(maska,round(Kn/2), round(Km/2),8);
    if maska1 == zeros(size(maska1)) % by victor
%         maska1 = bwselect(maska,8);
    end
% maska1 = maska;
    maska=bwmorph(maska1,'fill',4);
